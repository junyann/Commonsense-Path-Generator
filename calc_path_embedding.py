...
def run_generating(args):
    ...
    config = GPT2Config.from_pretrained(args.generator_type)
    ...
    gpt = GPT2Model.from_pretrained(args.generator_type)
    config.vocab_size = len(datahelper.gpt_tokenizer)
    gpt.resize_token_embeddings(len(datahelper.gpt_tokenizer))
    pretrain_generator_ckpt = os.path.join('/home/jun/project/Commonsense-Path-Generator/generator_ckpt', 'commonsense-path-generator_transformers_2.8.0.ckpt')
